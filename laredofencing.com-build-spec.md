...
- The finished repository must be a flat Astro project: `package.json`,
  ...
  repository root. There must be no `lead-gen-sites/sites/laredofencingcontractor.com/` wrapper,
...
- Deploy to Vercel project `laredofencingcontractor-com`
...
- Production domain: `laredofencingcontractor.com`
- Set `SITE_URL=https://laredofencingcontractor.com` for the build and replace the example URL in
  `astro.config.mjs`
...
- geo: `Laredo, Texas`
- domain: `laredofencingcontractor.com`
...
- Search Console operator handoff: create and verify the Domain property `sc-domain:laredofencingcontractor.com` and save that exact property identifier in the dashboard site record
...
- Before launch, the operator creates a Google Search Console Domain property for
  `laredofencingcontractor.com` and completes DNS verification. The exact dashboard/API property
  identifier should be `sc-domain:laredofencingcontractor.com`. Record the Search Console property URL,
...
4. Push this flat project to its own remote Git repository. Import that repository into
   the dedicated Vercel project `laredofencingcontractor-com`, keep Root Directory blank/`.`, and configure
   `laredofencingcontractor.com` in Vercel.
